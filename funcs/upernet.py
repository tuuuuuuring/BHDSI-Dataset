...
class ResNet(nn.Module):
    def __init__(
            self, block, layers,inch=6, num_classes=1000, zero_init_residual=False, groups=1,
        ...
        self.conv1 = nn.Conv2d(inch, self.inplanes, kernel_size=7, stride=2, padding=3, bias=False)  # 输入
...
class UPerNet(nn.Module):
    # def __init__(self, num_classes=1, inchi=6):
    def __init__(self, num_classes):
        super(UPerNet, self).__init__()
        self.num_classes = num_classes
        # self.backbone = ResNet.resnet50(inch=inchi, replace_stride_with_dilation=[1, 2, 4])
...
if __name__ == '__main__':
    input_tensor = torch.randn(2, 5, 256,256)
    sfd=UPerNet(1,5)
    # sdf=NewCRFDepth(version='tiny07')
    cbn = sfd(input_tensor)
    print(cbn.shape)
